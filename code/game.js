...
Level.prototype.obstacleAt = function (pos, size) {
  ...
  //Cosnsider the side and top and bottom as walls

if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

  //Check each grid position starting at yStart,. xStart to yend, x end
  //check for posible obstacles (non null value)
    for (var y=yStart; y<yEnd; y++)
    {
      for (var x=xStart; x<xEnd; x++)
      {
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Find if there is and obstical at the new position.
